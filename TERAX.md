...
- `git::commands::*`: full source-control surface (`git_status`, `git_diff`, `git_diff_content`, `git_stage`, `git_unstage`, `git_discard`, `git_commit`, `git_fetch`, `git_pull_ff_only`, `git_push`, `git_log`, `git_show_commit`, `git_commit_files`, `git_commit_file_diff`, `git_panel_snapshot`, `git_resolve_repo`, `git_remote_url`). All gated through the workspace authorization registry (skipped for SSH, where the explicit host connection is the capability — trust-the-host-account). `git::process::build_git_command` routes Local + WSL + SSH: the SSH arm runs git on the host over the shared ControlMaster as one POSIX-single-quoted remote command (`remote_git_command` + `quote_remote_arg`/`quote_remote_path`), reusing the same `run_git` timeout/output machinery; `git_diff_content` reads the unstaged worktree side via `ssh::read_file`. Remote worktree *creation* (`git_add_worktree`) is the one git op still not SSH-routed (needs remote home + mkdir).
- `tmux::*` (`tmux_list_sessions`, `tmux_kill_session`, `tmux_rename_session`): the tmux session picker (a quick-switcher modal that pops when an SSH tab connects to a host running tmux). Remote calls ride the terminal's live ControlMaster via `ssh::run_remote_capture` (one-shot text command, reuse-only) gated by `ssh::master_alive` (`ssh -O check`); local listing goes through `$SHELL -lc` so a GUI macOS app finds Homebrew tmux. Session names are allowlist-validated `[A-Za-z0-9_-]` so every single-quote splice is injection-safe. The attach launch (`tmux new-session -A -s`) is threaded through `pty_open` (`shell_init::build_ssh` tmux variant). `-F` output uses a printable `|` separator with the session name LAST (tmux escapes non-printable bytes, so a 0x1f comes back as literal `\037`).
...
- **tabs/** — `useTabs` is the source of truth for tab list + active id. `useWorkspaceCwd` derives explorer root + inherited cwd for new tabs from active tab. `basename` splits on both `/` and `\`. A macOS two-finger horizontal trackpad swipe switches tabs: detected **natively** in `install_tab_swipe_monitor` (`src-tauri/src/lib.rs`) — a local `NSEvent` scrollWheel monitor reads `phase`/`momentumPhase` (which WebKit never exposes to JS, so a wheel handler can't tell a swipe from its inertial tail and double-fires) and emits `terax:tab-swipe`; `useTabSwipe` listens and switches within the active space's strip order, deferring to a horizontal scroller / `data-no-tab-swipe` element under the cursor (`isInsideHorizontalScroller`).
